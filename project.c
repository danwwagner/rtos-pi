...
void read_encoder(void* arg) {
    ...
    if (flag == 1) {
        flag = 0;
        if (!last_enc_status && current_enc_status) count++;
        if (last_enc_status && !current_enc_status) count--;
    }
...
int main(int argc, char* argv[]) {
    ...
    // causes issues; next set to a periodic task every 4 seconds
    rt_task_set_periodic(&ir, TM_NOW, rt_timer_ns2ticks(4000000000));
